...
so a pod that mounts the SA secret and sets the standard var works with no app-specific config —
and `GCP_PROJECT_ID` can be omitted since it is read from the key JSON.
